...
# Evaluation loop
feature_selection_names = ['Mann-Whitney U', 'LASSO', 'RFE']
...
# 4. Voer de Nested CV uit! 
# De magie van scikit-learn: door 'grid_search' in 'cross_val_score' te stoppen,
# weet Python dat hij voor ELKE outer fold een compleet nieuwe grid_search moet draaien.
nested_scores = cross_val_score(grid_search, X_train_final, y_train, cv=outer_cv, scoring='accuracy')

# 5. Print de eerlijke, geneste resultaten
print("\n🏆 --- Nested CV Results --- 🏆")
print(f"Accuracies per outer fold: {nested_scores}")
print(f"Unbiased Mean CV Accuracy: {nested_scores.mean():.3f} (+/- {nested_scores.std():.3f})")

# 6. Train het uiteindelijke model voor productie/testset
# Nested CV geeft je een score, maar géén getraind model. Daarvoor fitten we 
# de GridSearchCV nog één keer op de héle trainset.
print("\n⚙️ Tuning final model on entire training set...")
grid_search.fit(X_train_final, y_train)
print(f"Best parameters for final model: {grid_search.best_params_}")

# Nu is grid_search.best_estimator_ klaar om voorspellingen te doen op je X_test_final!

# %%


# %% --- FINAL EVALUATION ON TEST SET ---
# Nu gebruiken we de 'held-out' testset die we aan het begin apart hebben gehouden.

# Doe voorspellingen met het allerbeste model (best_estimator_)
y_test_pred = grid_search.best_estimator_.predict(X_test_final)

# Bereken de uiteindelijke test accuracy
test_accuracy = accuracy_score(y_test, y_test_pred)

print("\n🔥 --- FINAL TEST SET RESULTS --- 🔥")
print(f"Final Accuracy on unseen test data: {test_accuracy:.3f}")

# Optioneel: bekijk welke klasses fout zijn gegaan
from sklearn.metrics import classification_report, confusion_matrix
print("\nClassification Report:")
print(classification_report(y_test, y_test_pred, target_names=['non-GIST', 'GIST']))
# %%
